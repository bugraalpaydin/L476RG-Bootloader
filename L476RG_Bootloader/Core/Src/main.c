...
int main(void)
{
  ...
  HAL_Init();
  //stm32l476xx.h'da buldum
  //USART2 interrupt enabled
  //NVIC->ISER[1] |= (1<<38);
  //programming manual'de buldum
  //USART2 interrupt priority is set to 0111
  //NVIC->ISPR[9] |= (1<<20);
  //NVIC->ISPR[9] |= (1<<21);
  //NVIC->ISPR[9] |= (1<<22);
  //NVIC->ISPR[9] &= ~(1<<23);
...
static void goto_application(void){
  char string[20];
  sprintf(string, "\n\rI am now in the goto_application\n\r");
...
void jump_to_application(void)
{
    ...
    // Reset handler'a (main uygulamaya) atla
